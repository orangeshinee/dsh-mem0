...
function renderMemories(results) {
 ...
 */
function resolveIds(args, config, opts = {}) {
    const ids = {};
    const value = config();
    if (args.user_id)
        ids.user_id = args.user_id;
    else if (value.defaultUserId)
        ids.user_id = value.defaultUserId;
    if (args.agent_id)
        ids.agent_id = args.agent_id;
    else if (opts.defaultAgent && value.defaultAgentId)
        ids.agent_id = value.defaultAgentId;
    if (args.run_id)
        ids.run_id = args.run_id;
    return ids;
}
...
function renderHistoryEntry(entry, index) {
    ...
    const change = entry.old_memory === null || entry.old_memory === undefined
        ? `-> ${entry.new_memory ?? '(empty)'}`
        : `${entry.old_memory} -> ${entry.new_memory ?? '(empty)'}`;
    return `${index + 1}. [${event}]${when}\n   ${change}`;
}
/** The add tool: `POST /memories`. */
export function mem0AddTool(client, config) {
    return defineTool({
        name: 'mem0_add',
        description: 'Store new memories into the self-hosted mem0. Provide either a single message string (messages, role defaults to "user") ' +
        ...
        async execute(args) {
            try {
                const messages = typeof args.messages === 'string'
...
export function mem0SearchTool(client, config) {
    return defineTool({
        name: 'mem0_search',
        description: 'Semantic search over memories stored in the self-hosted mem0. Returns the closest memories with relevance scores. ' +
        ...
        output: {
            ...
            render: (args, value) => text(value.ok
                ? `search "${String(args.query ?? '')}" -> ${value.count ?? 0} result(s):\n${renderMemories(value.results)}`
...
export function mem0GetTool(client, config) {
    return defineTool({
        name: 'mem0_get',
        description: 'Read memories from the self-hosted mem0. With id, returns that one memory; without id, lists memories ' +
...
export function mem0UpdateTool(client) {
    return defineTool({
        name: 'mem0_update',
        description: 'Update a stored memory in the self-hosted mem0 by id (use mem0_search or mem0_get to find the id first). ' +
        ...
        output: {
            ...
            render: (args, value) => text(value.ok
                ? `updated memory ${String(args.id ?? '')}: ${value.message ?? 'ok'}`
        ...
        async execute(args) {
            try {
                ...
                return { ok: true, message: typeof response === 'object' && response && 'message' in response ? String(response.message) ?? '' : '' };
            }
            catch (error) {
...
export function mem0DeleteTool(client, config) {
    return defineTool({
        name: 'mem0_delete',
        description: 'Delete memories from the self-hosted mem0. With id, deletes that one memory. Without id, deletes ALL memories of the ' +
        ...
        output: {
            ...
            render: (args, value) => text(value.ok
                ? `deleted ${String(args.id ?? '(scoped batch)')}: ${value.message ?? 'ok'}`
...
export function mem0HistoryTool(client) {
    return defineTool({
        name: 'mem0_history',
        description: 'Show the edit history of one memory in the self-hosted mem0 (created_at, updated_at, previous values). ' +
        ...
        async execute(args) {
            try {
                ...
                return { ok: true, id: args.id, history: asJson(history) };
            }
            catch (error) {
                return failure(error);
            }
...
export function mem0ResetTool(client) {
    return defineTool({
        name: 'mem0_reset',
        description: 'Wipe ALL memories from the self-hosted mem0. Destructive and irreversible; requires admin server role ' +
...
export function mem0StatusTool(client) {
    return defineTool({
        name: 'mem0_status',
        description: 'Check the self-hosted mem0 connection: server reachability, auth state (needs an apiKey), and the current ' +
        ...
        output: {
            ...
            render: (_args, value) => {
                if (!value.ok)
                    return text(`mem0_status failed: ${value.error ?? 'unknown error'}`);
                const lines = [
                ...
                if (value.configure !== undefined)
                    lines.push(`configure: ${JSON.stringify(value.configure)}`);
        ...
        async execute() {
            try {
                ...
                return {
                    ...
                    setupStatus: (status.setupStatus ?? null),
                    authError: status.authError ?? '',
                    ...(status.configure !== undefined ? { configure: status.configure } : {}),
                };
            }
            catch (error) {
